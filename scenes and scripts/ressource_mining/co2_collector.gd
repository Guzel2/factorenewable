...
var production_per_tile = .5 #this amount gets produced per tile, per second

var power_consumption = .75
...
func _ready():
	...
	production_time = (1.0/production_per_tile)/4
...
func _process(delta):
	...
	if inventory['co2'] < inventory_size and production_time > 0 and is_powered:
	...
	if inventory['co2'] > 0:
		if output_neighbors.size() > 0:
			...
			while searching_for_output < 8:
				var moved_item = output_neighbors[0].receive_item('co2', 1, self)
